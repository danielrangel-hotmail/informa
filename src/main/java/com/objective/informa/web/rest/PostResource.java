...
/**
 * REST controller for managing {@link com.objective.informa.domain.Post}.
 */
@RestController
@RequestMapping("/api")
public class PostResource {
    ...
    /**
     * {@code PUT  /posts-publish} : Publica um post
     *
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/posts-publica")
    public ResponseEntity<PostDTO> publicaPost(@Valid @RequestBody SimplePostDTO simplePostDTO) throws URISyntaxException {
        log.debug("REST request to update Post : {}", simplePostDTO);
        if (simplePostDTO.getId() == null || simplePostDTO.getVersao() == null) {
            throw new BadRequestAlertException("Invalid id or version", ENTITY_NAME, "idnull");
        }
        PostDTO result = null;
        try {
            result = postService.publica(simplePostDTO);
            return ResponseEntity.ok()
            ...
            throw new BadRequestAlertException(e.getMessage(), ENTITY_NAME, simplePostDTO.toString());
        }

    }

    /**
     * {@code GET  /posts-user} : get all the posts.
     *
    ...
    public ResponseEntity<List<PostDTO>> getPosts(Pageable pageable) {
        ...
        Page<PostDTO> page = postService.findAllPublicados(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
